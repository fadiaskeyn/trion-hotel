<template>
    ...
    <thead class="bg-gray-200">
        <tr>
            ...
            <th class="px-4 py-2 border no-print">Bukti</th>
            <th class="px-4 py-2 border no-print">Opsi</th>
        </tr>
    </thead>
    <tbody>
...
function cetakpdf() {
    ...
    const table = document.querySelector('table[name="table"]').cloneNode(true);

    // Hapus kolom dan baris dengan kelas no-print
    const noPrintElements = table.querySelectorAll('.no-print');
    noPrintElements.forEach(el => el.parentNode.removeChild(el));

    // Hapus kolom header yang terkait
    const noPrintHeaders = table.querySelectorAll('thead th.no-print');
    noPrintHeaders.forEach(el => el.parentNode.removeChild(el));
